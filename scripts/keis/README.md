...
- 熱門時段（早上開盤 06:00-10:00、晚上同業活躍 18:00-24:00）→ 每 ~5 秒（帶隨機抖動 ±3s）掃一次，逮到符合條件的新名單**立刻搶 + 推 LINE**
- 一般時段（白天 10:00-18:00）→ 1 分鐘一次
- 深夜（00:00-06:00）→ 30 分鐘一次，純安全網
...
- **待辦**：熱門時段的起訖邊界（06:00-10:00 / 18:00-24:00）只有 1~2 天樣本，是先射箭再畫靶，等 `logs/` + `page1_track.csv` 累積多天真實資料後應收斂調整。
