...
- 每天換日瞬間 → **一定推一則今日戰果保底通知**（掛 0 那天也有訊息，分辨貨少 vs 搶輸 vs 系統掛掉），並回查 `my-applications` 對帳補回漏記的
- IP 被擋（離開門市網路）→ 推 LINE 提醒（30 分鐘節流）並持續重試
- **斷網退避**：暫時性錯誤（逾時等）預設每 3~8 秒重試；連續失敗 ≥10 次（判斷是真的斷網而非開盤塞車）自動拉長到每 5 分鐘重試一次，並推 LINE 告知；恢復時再推一次，別整夜每幾秒瘋狂重試灌爆 log
