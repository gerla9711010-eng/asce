...
# ⚠️ 預設關閉：使用者 2026-07-15 明確說過「不需要挖舊存量，只顧每天新放出來的」。
#    07-23 重新盤點發現同業其實天天在挖（每天 50~90 筆、名次挖到 1300），而我方配額
#    長期只用不到一半，所以把功能做好放著——要不要開是使用者的決定，改成 True 即可。
POOL_FILL_ENABLED = False     # True = 晚上拿剩餘配額從池內存貨補；False = 只搶當天釋出的新貨
