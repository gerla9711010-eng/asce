...
DEEP_SWEEP_SEC = 600          # 每隔幾秒做一次「全池分頁掃描」（目前約 16 頁 1600 筆）
DEEP_SWEEP_MAX_PAGES = 25     # 全池掃描最多翻幾頁（每頁 100 筆），防呆用
...
FRESH_MAX_AGE_DAYS = 14       # 絕對上限：建檔超過 N 天一律不搶（總帳萬一遺失時的保險）

# ====== 用「建檔日期」判斷新舊（2026-07-23 用 111 筆實測觀測資料校正）======
# appearances.csv 累積的 111 筆「第一次看到某單號」的紀錄拿去對建檔時間，規律非常乾淨：
...
# 反過來，二手回鍋貨最快也要 建檔 →(最快 0.58 天)→ 釋出 → 被申請 →(7 天冷卻)→ 回鍋
#   = 建檔至少 7.58 天前，實務上多半 14 天以上。
# 這兩個區間不重疊，所以「建檔 7.5 天」是一條乾淨的分界線。
# 另一個實測到的重點：**還沒釋出的名單根本不在 query 結果裡**（建檔 07-19~07-21 的
# 一筆都查不到），所以「第一次出現 = 剛釋出」，這也是總帳判定成立的前提。
BASELINE_RESCUE_DAYS = 7.5    # 建總帳當下就在池子裡的「基準快照」名單，建檔在這天數內的
                              # 仍視為真新單（那批必然是這一兩天才釋出的），其餘一律不搶
...
def is_truly_new(inv: dict, rec: dict) -> bool:
    """是不是「真新單」——第一次進我們視野時就是沒人碰過的 Available。

    兩道判準（總帳為主、建檔日期為輔，理由見 CONFIG 的「用建檔日期判斷新舊」）：
      曾冷卻=Y            → 一定是二手貨（別人或我方拿過），不搶
      沒紀錄 / 新出現      → 剛釋出的真新單（未釋出的名單不會出現在 query 裡），可搶
      基準快照             → 來歷不明，但**建檔在 7.5 天內的救回來**：實測新單第一次
                            出現時建檔最多 6.98 天，二手回鍋最快也要 7.58 天，
                            兩者不重疊——這樣重開機建基準的那天才不會整天空轉。"""
    row = inv.get(rec.get("summary_id"))
    if row is None:
        return True                                   # 沒紀錄=這一刻剛冒出來
    if str(row.get("曾冷卻", "")).upper() == "Y":
        return False
    if row.get("來源") == "基準快照":
        return age_days(rec) <= BASELINE_RESCUE_DAYS
        ...
        try:
            ...
            if not inventory_ready:
                ...
                for r in newly_seen:                     # 每個新編號都留一行 log，事後可追
                    log(f"📒 新編號 id{r.get('summary_id')} 首次看到＝{r.get('status')}"
