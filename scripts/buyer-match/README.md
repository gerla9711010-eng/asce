...
結果有三個入口，**版面只有一份**：`build_static_view.py`（2026-08-05 整併，改一次三邊一起變）。
資料一律讀 `output/` + `manifest.json`，所以三邊內容一樣。

| 入口 | 怎麼開 | 誰產的 | 特點 |
|---|---|---|---|
| **localhost:5001** | 瀏覽器書籤（工作排程 `buyer-match-webview` 開機常駐） | `webview_server.py` → 每次請求現叫 `render()` | **即時**：重新整理就是最新資料；頂層視窗，**點連結會原地跳出預覽視窗** |
| **桌面 `配案看板.html`** | 雙擊 | 排程每天早上跑完重產一份 | 單檔可帶著走、離線可看；一樣有原地預覽 |
| **手機看板（Claude Artifact）** | 手機書籤 | `--format artifact`，**要人工重新發布**才會更新 | 包在 iframe 裡，外站連結只能開新分頁或按「複製連結」 |

版面規則（都在 `build_static_view.py`）：
- 物件固定排序 **🆕 新上架 → 🔻 降價 → 其他**（`TAG_ORDER`），打開就先看到今天能打電話的
- 客戶卡右上角標「新 N／降 N」，不用一張張點進去找
- 螢幕 ≥760px 時字自動放大一級（電腦看不吃力），手機維持原尺寸

⚠️ `webview_server.py` **不要再自己寫 HTML**。以前它有一份自己的條列式版面，
結果只改 `build_static_view.py` 那邊、使用者天天在看的 5001 沒跟著變（2026-08-05 踩到）。
...
| 選法 | 行為 |
|---|---|
| 群組＝**（★ 全部群組：A買→B買→C買 依序跑完）** | 固定只跑 **A買→B買→C買** 三組（`MAIN_GROUPS`）。房地上的「其他」之類雜項資料夾**不會跑**——那不是要配的買方名單，2026-08-04 跑進去過，白花幾十分鐘。要跑那些就在群組下拉單獨選。客戶／客需下拉會變灰，因為每一組的每個人都會跑。粗估每組 45 分鐘，確認視窗會把時數算給你看 |
...
某一組炸掉會記下來繼續跑下一組（三組要跑好幾小時，不該被第一組拖掉），LINE 總表
會標「❌ 整組沒跑成：X買」。想只跑一組就 `--group B買`。
...
3. 依序跑 A買→B買→C買（正式模式，有分享連結），每個「客戶／子條件」各自存檔在 `output/`
