...
工作排程 `houseol-auto-clockin` 每週四/六/日 09:00 觸發 + `--jitter 3600`（0–60 分**常態分佈**，中心點 30 分，不是均勻分佈，避免常常抽到太早或壓在邊界）。

## 它怎麼運作
- **Windows 工作排程器** 每週四/六/日 09:00 觸發，`clockin.py --jitter 3600` 用常態分佈（`random.gauss`，中心 30 分、std 10 分，超出 0~60 分範圍會截斷）決定延遲多久 → 落在 9:00–10:00 之間，且大多集中在中段，每次時間不一樣。
