...
- **主建物 E 欄樓層名稱**（2026-07-15 修）：逐層列出坪數的同時，E12/E14/E16/E18/E20 也會依謄本實際層次名稱改寫（一層/二層/三層/四層/突出物一層…，`_clean_floor_label()` 去掉謄本「突出物一層：２５．７２平方公尺」這種格式裡冒號後面重複的面積字樣）。只有真的有多層（`floor_pings` 筆數 >1）才會覆蓋，一般大樓單一樓層戶 E12 仍維持範本預設「室內」。
...
- **地上權（`I38`）目前完全沒有判斷邏輯**：程式只有「全部（`B36`）／持分（`B38`）」兩種，依土地謄本的 `land_share` 決定，沒有謄本資料時預設「持分」；`I38` 地上權從來沒有任何程式路徑會去勾。而且目前「建物案」流程強制要求選土地謄本才能跑（`if not land_paths or not bldg_paths: 請先選擇土地與建物謄本 PDF`），地上權物件本來就沒有土地謄本，等於**目前這個工具跑不了純地上權案件**，要手動事後在 Excel 上改。要支援的話需要另外加「本案為地上權」的勾選/流程分支，目前還沒做。
