...
最後更新：2026-07-26（搶單早上高頻檔提前到 07:00 修睡過頭空窗、回報改三段式；自動簽到 jitter 改常態分佈避免常常抽到太早/壓線）
...
**外部呼叫都有重試了（2026-07-25）**：線 A/線 C 的 KEIS、Notion、Gemini、FB 上傳節點全部設 `retryOnFail=3 次 / 間隔 5 秒`。起因是 07-24 掉了兩班（13:00 `KEIS 下載照片zip` ECONNABORTED、19:00 `查 Notion 是否已建` HTTP 522），都是對方伺服器抖一下就整條線陣亡。⚠️ **`FB 發貼文`／`Notion 建待發列`／`KEIS 新增廣告`／`刪舊貼文` 刻意不加重試**——這四個重打會產生重複貼文/重複資料，寧可失敗也不要重複。

### ▶ 下次開工從這裡接

1. **驗專員電話：✅ 已確認成功**（07-26 09:00 那班，`查專員電話` 節點回傳「0937565158／本人手機」，且已寫入 Notion）。
