...
🟢 **08-29 修**：健康迴圈判活條件有洞，失敗一次後永遠不再重試，卡死 2 小時多才被發現。
已修好+手動救回（新網址已寫回 n8n）。經過見 `incidents.md`。**還沒被下一次真的斷線驗過**。

🟢 **`scripts/incident-watch/watch.py`**：背景常駐，每 15 分鐘查 n8n 執行紀錄，撞到這個已知
「通道死掉」案例會自動修+發 Telegram 報告，其他失敗照樣走一般告警。
⚠️ 09-02 才發現先前寫的「已裝開機自動啟動」是假的（`shell:startup` 裡根本沒有捷徑，
08-29～09-02 完全沒在跑），**09-02 已補裝並啟動**。以後裝完自動化要去 `shell:startup` 實地看一眼。
