...
- **FB 權杖輪替進行中**（🔴 優先）：照 `docs/fb-token-rotate.md`。已做完步驟 1-2（舊系統使用者權杖已撤銷、新的已產生，App/效期/4 個權限都設對）。**卡在步驟 3**：新系統使用者權杖只顯示在瀏覽器畫面上一次，沒人複製存起來——回電腦前先去 Meta 後台系統使用者頁看畫面還在不在；不在的話回 `docs/fb-token-rotate.md` 步驟 1-2 重跑一次（撤銷+重產生，2 分鐘，因為還沒開始用不影響任何東西）。跟 Claude 說「繼續權杖輪替」即可接手。
