...
**待決定：「n8n 整個掛掉」現在沒有獨立偵測**——grab.py 那條最後防線本來就因為
`.env` 拔掉 `KEIS_HEARTBEAT_WEBHOOK` 而是死的，只能靠工作回報/戰果查詢打不通間接發現。
要重建的話，建議別再靠 n8n 排程自己檢查靜態時間戳（就是這次假警報的殭屍狀態根源），
改成：n8n 那邊只要一個「收到 POST 就回 200」的極簡 webhook，真正的「連續失敗多久才升級
告警」判斷交給 grab.py 自己做（`heartbeat_alert_sent` 那段邏輯本來就寫好了，只是沒接上）。
