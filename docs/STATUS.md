...
| **售屋表填寫** `scripts/sale-form/` | 🟢 2026-07-18 健檢全三批完成（工單：桌面 `售屋表v3.5健檢評估_工作單.md`）。第1+2批：租賃整數格式/月租標籤/貸款確認三步/車位預填/choice防誤跳+數字鍵/總覽確認頁/執行緒安全。第3批：塗銷他項權利不誤加、存檔失敗友善提示、土地表鑰匙編號+專員電話、vbs 真無視窗+crash.log、重編上一案、開啟上次檔案鈕、金額防呆、進度計數修正、死碼打掃（W3 單價查證過不用修：範本是 TRUNC(...,4)）。**桌面已同步**、真範本+邏輯測試全過。⚠️ 待門市拿真實案件實測（清單在工單頂部）；塗銷防護只用模擬文字驗過，遇真實含塗銷謄本要核對 log | `scripts/sale-form/README.md` |
