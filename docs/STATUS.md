...
最後更新：2026-07-29（搶單補回不再誤記值班分派名單，本機帳已清、Notion 刻意不動）
...
| **買方配案** `scripts/buyer-match/buyer_match.py` | 🟢 2026-07-28 門市電腦實跑驗證過（含詳情頁/分享連結/剪貼簿全套）。依條件（行政區可多選＋車位含平面/機械走網站真下拉；社區/路名關鍵字＋總價＋房數走文字/程式過濾）掃 i智慧Pro 流通物件，逐筆抓專員聯絡方式＋i智慧公開分享連結，整段文字印出＋存檔＋複製到剪貼簿。內建 Tkinter GUI（雙擊 `啟動工具.vbs`）+ CLI 兩種用法，CDP attach 不存帳密，選填 `.env` 可過期自動重新登入。客戶名單／推播對象仍由使用者自己看 Notion 手動決定。細節與踩過的坑看 README | `scripts/buyer-match/README.md` |
...
4. **PG0092558（仁武預售）的 KEIS 廣告追蹤還缺一筆**：2026-07-29 17:00 那班 FB 發了但 KEIS 建檔失敗（見下方門檻改版），使用者說先放著、等他確認後再指示怎麼補。要補的話連結是 `https://buy.yungching.com.tw/house/7454949`。同一天 09:00 的 YG0092410 也缺，但那件 `show_in_web=false`（根本沒上官網，也沒有官網連結可用）。
5. Notion 裡另外 3 筆 v2 時期的舊資料（YC1868705 / YE0095535 / YC1835328）不在 KEIS，線 B 每天都會掃到它們然後略過。其中 YC1835328 已判定下架，之後會被標成下架（那筆沒有粉專貼文，不會刪到東西）。
