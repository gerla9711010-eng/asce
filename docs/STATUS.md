...
- ⚠️ **到 2026-07-25 為止仍未接進線 A**。要接的位置：`白名單清洗` 之後、`Notion 建待發列` 之前
- Notion「專員電話」欄只有 07-22～07-23 那幾筆有值，是當時實測端點時手動回填的；**07-24 起全空就是因為這段從來沒進 workflow**。查過線上 19 支 workflow 與執行紀錄（exec 1692）都沒出現過那些號碼，不是壞掉，是沒做
...
1. **專員電話接進線 A**：credential `展售系統帳密（自動登入）`（id `ZkOT0wWz3oZTpdME`）已建好、登入流程已實測通過，**但還沒接進線 A**。接點：`白名單清洗` 之後、`Notion 建待發列` 之前，抓到就寫 `專員電話` 欄。做法見上方「專員電話來源」。線 C（重發輪替）同一段也要接。
