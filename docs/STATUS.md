...
- **搶單「補回」通知已改寫（2026-07-29）**：跨日/啟動回查補回的名單，LINE 訊息原本寫死「今日剩餘配額 0」
  （實際是 3 帳號各 7、共 21 全滿），且看不出那筆不是機器人搶的。已改成真查配額 + 標明「不是機器人搶的」
  + 非手機號碼標 ☎️市話。n8n 那支（`keis-grab-notify`）已部署並重啟。
  ⚠️ **剩一步：門市電腦上的 watch 要重啟一次**（現在跑的 PID 還是舊 grab.py），關掉黑視窗再點一次 run.bat 就好。
  背景：07-29 00:00 補回的 79300（陳太太 077252509）是**有人用薛力瑜帳號在 KEIS 網頁手動申請**的，
  機器人全程沒看過這個編號（`is_mobile()` 只放行 09/9 開頭，市話一定被擋）。系統沒壞。
