...
| **買方配案** `scripts/buyer-match/buyer_match.py` | 🟢 2026-07-28 門市電腦實跑驗證過（--list-only 掃到 450 筆、篩選 73 筆；正式模式 2 筆詳情頁+分享連結+剪貼簿全過）。依條件（行政區/社區/路名關鍵字＋總價＋房數）掃 i智慧Pro 流通物件，逐筆開詳情頁抓「承辦人聯絡資訊」＋點「分享」拿 i智慧公開分享連結（`www.ycut.com.tw/case-info/{uuid}`，免登入可直接傳客戶），整段文字印出＋存檔＋複製到剪貼簿。內建 Tkinter GUI（雙擊 `啟動工具.vbs`）+ CLI 兩種用法。CDP attach 手法跟桌面「自動比對專約」工具同款（獨立 profile Chrome port 9223，不存帳密），選填 `.env` `ISMART_ACCOUNT`/`ISMART_PASSWORD` 可在 session 過期時自動重新登入（已實測成功）。客戶名單／推播對象仍由使用者自己看 Notion 手動決定，這支只負責產出配對好的物件文字。**除錯踩過的坑**（都已修好、細節見 git log）：①i智慧登入頁是 client-side redirect，`domcontentloaded` 那瞬間 URL 可能還沒變，要多等再判斷過期；②「分享」按鈕是 Angular 自訂元件，JS `el.click()` 點得到但頁面沒反應，要改用 Playwright 真滑鼠 click；③點「分享」不是跳原生 confirm()，是站內「提醒」對話框，要再點框裡的「確定」才會真的開新分頁 | `scripts/buyer-match/README.md` |
