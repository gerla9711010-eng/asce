...
⚠️ **`KEIS 心跳檢查` 這支已經再度停用，別再重新啟用**：08-25 收工時把它跟另外 3 支一起
重新啟用，結果它是 08-24 剛被 `incidents.md` 記錄過的死流程——grab.py 早就不打這個 webhook
了（心跳監控搬去 grab.py 本地計時器＋Telegram 直推），一啟用就照抄舊假警報「310 小時沒心跳」。
已經 deactivate 回去。**這支永久不用了，之後做 Telegram 遷移一類的批次作業前，先看
`incidents.md` 有沒有記過這個 workflow 是死的。**
...
| 廣告發文看門狗／自動簽到／市場週報／行情看板更新／**KEIS 心跳檢查** | ⚪ 已停（心跳檢查是永久性的，其他沒動）|
