...
**LINE 額度控管（拍板：不升付費方案，200 則/月內解決，涵蓋全部工具不只廣告系統）**：
- 訊息三級制：A 級（下架、故障、搶單成功）→ 即時推播；B 級（待聯絡、簽到、行情）→ 每日 1 則合併摘要；C 級（各種成功/完成紀錄）→ 只寫 Notion「系統動態」頁，永不推播
- 使用者主動觸發的對話（建檔預覽、發布確認、社團記錄）一律用 Reply（免費無上限），不用 Push
- 守門員：推播前先查 LINE quota consumption API。>160 則（80%）→ B 級停推改寫 Notion；>190 則 → 只剩故障+下架能推。每月 1 號摘要附上月用量
- 退路（真不夠再啟用，全部零元）：B 級改走 Notion @mention 通知（Notion App 免費推播）→ LINE 只留故障+下架
