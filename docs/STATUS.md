...
- **卡點：要登入 session**。這是 ASP.NET WebForms（ViewState），n8n 要自己跑 login POST。**帳密還沒拿到**，使用者說會另外提供、不要寫死（放 n8n credential，別進 git）
...
|---|---|---|
| `yc-v3-scan-publish.json`「廣告v3 掃描發文線」 | 線 A 掃描+發文+**同步 KEIS 廣告追蹤**（cron 09/11/13/15/17/19，每次 1 件） | 🔴 **2026-07-23 17:40 已停用**（使用者指示）。原因：KEIS 砍 API 後抓不到照片，每輪空轉。修好照片來源再開回來 |
...
0. 🔴 **KEIS 砍 API（2026-07-23 下午）**：在售列表 40→18 欄、物件詳情 149→61 欄，**`images` 照片網址整組不見**，`official_url`／`age`／`school_info`／`layout`／`floor_info` 也掉了（`seller_*` 屋主個資也一併被砍，研判是刻意收緊而非故障）。線 A 因此每輪空轉，**已停用**。賣點 `feature_1~5`、捷運/商圈/公園、`sales_agent_name`／`store_name` 都還在。改善方案四塊（止血閘門／照片改抓官網或展售系統／用 room_count 等組回格局樓層／加 API 體檢告警）已提給使用者，**他說要換個方式，等他決定**。⚠️ 線 C 吃同一支詳情 API，會壞在同一個地方。
1. **專員電話自動化**：端點已找到（見上方「專員電話來源」），**等使用者提供展售系統帳密**放進 n8n credential，就能讓線 A 建列時順手把專員手機一起帶入。
