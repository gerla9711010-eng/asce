...
| 工具 | 狀態 | 深度文件 |
|---|---|---|
| **公買搶單** `scripts/keis/grab.py` | 🟢 已上線（門市電腦）。**2026-07-23 大修（定義定案，不要再改）**：「最新名單」＝**這個 summary_id 從沒出現在我們的完整總帳 `inventory.csv` 裡**（＝剛進池），就這一條，不摻建檔日期、不摻編號大小。三帳號交叉觀測證明：同樣建檔 07-16 的名單分散在兩個編號帶（77192/77196 是 07-20 18:00 進池、74251~74619 是 07-16 18:00 進池躺 7 天），所以建檔日期會誤判；同批進池編號橫跨區間，所以「編號>當日最大號」會整批漏記（舊 appearances.csv 就是這樣漏掉 74xxx）。配套：①窗口 20→100 + 每 10 分鐘全池分頁掃描（掃不完整＝「沒看過」會被污染）②全池掃描輪流換帳號（帳號看不到自己申請的，實測三視角 2823/2835/2836、聯集 2856）③`inventory.csv` 每個編號一行（首次看到/首次狀態/來源/曾冷卻/符合篩選/不符原因/我方動作），觀測與搶單共用同一條判定④二手回鍋照擋（名單被申請滿 7 天到期會變回 Available 且 app_time 被清空，我方 8 天前搶的 53 筆有 16 筆正躺在池子裡裝新的）。⚠️ 建基準當天等於不搶，隔天起正常。2026-07-22 加品質控管篩選（只搶手機/排除公寓/預算上限<1000萬不搶/記行政區）+ 穩定性加固（主帳號逾時自動換帳號查、400自動重登、觀測檔搬離OneDrive）；分層時段改為 07:30-10:00／10:00-17:30／17:30-24:00／00:00-07:30(等同停止)。Notion 搶單名單 DB「行政區」欄位已用 API 補上 | `scripts/keis/README.md`、`docs/keis-grab-hardening-and-filters.md` |
