...
**（非廣告系統）房地/i智慧配案的下一步**（2026-07-29 收工時講好、都還沒做）：

1. **★ 排程每天 08:10 自動跑（規格已談定，下次直接做，不用再問使用者）**
   - **只跑 A買**（B買/C買 之後想加再改）；跑正式的（要分享連結，不是 dry-run）
   - **通知走 LINE**：比照 `scripts/keis/grab.py` 的 `KEIS_NOTIFY_WEBHOOK` n8n webhook 手法，
     推「哪位客戶幾筆」的總表；**失敗或房地沒登入也要推**
   - **⚠️ 必須先解決：房地沒有自動重登**（i智慧 有 `.env` 帳密可自動登入，房地沒有）。
     排程前要加一道登入檢查：偵測到房地被導去 `accounts/login` 就推 LINE 叫人去登、
     並中止這次執行，**不要靜靜撈到 0 筆**（這是這個 repo 一再踩到的「流程回報成功≠事情做到了」）
   - 排程手法照 `scripts/clockin/install-task.ps1`（Windows 工作排程 + pythonw）。
     ⚠️ 那支 ps1 檔案本身要保持純 ASCII，Windows PowerShell 5.1 會弄壞非 ASCII 原始碼
   - 還要處理：CDP Chrome（port 9223）沒開著的話要自己啟動；門市 00:00~約07:22 斷網，
     08:10 已恢復所以時間點沒問題
   - 使用者 2026-07-29 說「流量不夠，下次弄」，所以只留規格沒實作
2. **GUI 整併成三層下拉**（使用者說手打客戶名字「不人性化」）：群組／客戶／客需三個下拉直接從
   房地讀出來給他點，不要再手打（名字很長又會打錯）。
   ⚠️ 「客戶留空＝整組跑」已經在 2026-07-29 PR #155 做掉了，剩下的是「改成下拉、不用打字」。
3. **「上次看過的不要再出現」**：存一份已推過的案件指紋，重跑只回報新出現的。使用者當下選「先不用」，
   但這是最可能下一個會想要的（做法可參考 `scripts/keis/grab.py` 的防重複）。
4. 車位型態（平面式/機械式）還沒對應過去，目前只做到「有/無車位」。
