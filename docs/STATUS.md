...
| **買方配案** `scripts/buyer-match/buyer_match.py` | 🟡 2026-07-27 新建，**還沒實跑過**（開發時只在 Claude 瀏覽器裡手動驗證過搜尋/詳情頁/分享連結三段機制，Playwright 腳本本身待門市電腦跑第一次）。依條件（行政區/社區/路名關鍵字＋總價＋房數）掃 i智慧Pro 流通物件，逐筆開詳情頁抓「承辦人聯絡資訊」＋點「分享」拿 i智慧公開分享連結（`www.ycut.com.tw/case-info/{uuid}`，免登入可直接傳客戶），整段文字印出＋存檔＋複製到剪貼簿。CDP attach 手法跟桌面「自動比對專約」工具同款（`open_real_chrome.bat` 開帶 port 9223 的獨立 profile Chrome，手動登入一次 i智慧，不存帳密）。客戶名單／推播對象仍由使用者自己看 Notion 手動決定，這支只負責產出配對好的物件文字。⚠️ 待辦：門市電腦跑一次 `open_real_chrome.bat`＋登入＋`python buyer_match.py --area <區域> --price-max <萬> --list-only` 先驗證篩選抓得對，再拿掉 `--list-only` 驗證詳情頁/分享連結/剪貼簿 | `scripts/buyer-match/README.md` |
