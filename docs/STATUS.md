...
訊息——排了 `/loop` 每 15 分鐘查一次 n8n 執行紀錄，撞到這個已知案例會自動修+發 Telegram 報告，
沒事就完全不動作。⚠️ 這個排程是 **session-only**（跑在這次對話的 CronCreate，關掉這個視窗
或 7 天到期就會消失，不會自動恢復），要長期運作得找一個常駐視窗重排，或改用 `/schedule`
（雲端排程）換成不受視窗限制的版本。目前只認得這一種案例，其他失敗照樣走原本的 Telegram 告警。
