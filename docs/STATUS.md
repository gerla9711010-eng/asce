...
| 工具 | 狀態 | 深度文件 |
|---|---|---|
| **公買搶單** `scripts/keis/grab.py` | 🟢 已上線（門市電腦）。**2026-07-23 定案（走了冤枉路，結論寫在這別再重來）**：搶單規則＝**只看編號最大的前 40 筆窗口**（`WINDOW_SIZE`）＋**建檔超過 10 天不搶**（`MAX_AGE_DAYS`，只當否決權：建檔舊→一定不是新單；建檔新→不代表是新單）＋二手回鍋保險（總帳看過被誰拿過的不搶）。**窗口本身就是把關**——池子照編號由大到小排，最前面幾乎必然是剛進池的。當天曾把窗口拉到 100+全池掃描並改用「總帳沒看過＝新」判定，10 分鐘就誤搶一筆建檔 2026-01-13 的老案（三帳號看到的池子不一樣：2823/2835/2836，「沒看過」常只是「這帳號看不到」），已回退。全池掃描保留但降到每小時一次、輪流換帳號，**只寫 `inventory.csv` 給人稽核，絕不進搶單流程**（總帳欄位：首次看到/首次狀態/來源/曾冷卻/符合篩選/不符原因/我方動作）。2026-07-22 加品質控管篩選（只搶手機/排除公寓/預算上限<1000萬不搶/記行政區）+ 穩定性加固（主帳號逾時自動換帳號查、400自動重登、觀測檔搬離OneDrive）；分層時段改為 07:30-10:00／10:00-17:30／17:30-24:00／00:00-07:30(等同停止)。Notion 搶單名單 DB「行政區」欄位已用 API 補上 | `scripts/keis/README.md`、`docs/keis-grab-hardening-and-filters.md` |
