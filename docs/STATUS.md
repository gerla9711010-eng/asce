...
- **GUI 整併成三層下拉**（使用者說現在「不人性化」）：群組／客戶／客需三個下拉直接從房地讀出來給他點，
  不要再手打客戶名字（名字很長又會打錯），「客戶=全部」就等於整組跑，單一與整組合成同一個介面。
  現在有 4 個入口（`房地i智慧配案.vbs` / `啟動工具.vbs` / `run_customer_match.py` / `run_group_match.py`）確實太亂。
- **「上次看過的不要再出現」**：存一份已推過的案件指紋，重跑只回報新出現的。使用者當下選「先不用」，
  但這是最可能下一個會想要的（做法可參考 `scripts/keis/grab.py` 的防重複）。
- 房地目前**沒有** `.env` 自動重登（i智慧有），登入過期要人工點進去登一次——想排程自動跑的話這關要先解決。
- 車位型態（平面式/機械式）還沒對應過去，目前只做到「有/無車位」。
