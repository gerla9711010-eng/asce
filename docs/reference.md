...
⚠️ `工作回報查詢`（`mKgzwywqPX2h70pR`）原本兩個查詢節點寫死篩選「今天 00:00 起」，導致同一天
問第二次、第三次會把已經回報過的舊資料整批重講一次（2026-08-14 發現）。已改成用 n8n workflow
static data 存「上次查詢時間」當篩選起點，LINE 回覆成功後才推進游標（失敗不推進，下次還抓得到）。
`讀取查詢游標`／`更新查詢游標` 兩個節點就是做這件事，改動時注意這條鏈別斷掉。

**LINE 推播額度精簡（2026-08-14）**：每月 200 則配額被廣告系統改版前的 11 個 push 節點燒最兇，
已拔掉（見上）。順手把「例行成功也會推播」的兩個也改成寫進系統日誌，不再主動 push：
`keis-grab-notify.json`（搶單成功通知）、`clockin-notify.json`（houseol 簽到通知），
來源分別記 `KEIS搶單`／`houseol簽到`（Notion select 新值會自動建立，不用手動加）。
兩支的 `組 LINE 訊息` 節點沒動，只是把尾巴的 `LINE Push` 換成 `寫入系統日誌`（POST
`/v1/pages` 到系統日誌 DB，同一套寫法）。`工作回報` 查詢本來就不篩來源，這兩類事件會
...
**待聯絡提醒／靜默失敗巡邏／情資週報（2026-08-25 補完）**：這幾支原本就是
push 型（不是系統日誌型），只是 LINE 節點還沒換 Telegram、而且處於停用。用
`scripts/n8n_line_to_telegram.py` 換節點，再 deactivate+activate（情資週報）／activate
（另外的，本來就是關的）。換裝前先加防吵，不然開回去會洗版：
- **靜默失敗巡邏**：`找出被吞掉的錯誤` 原本只在同一批次內去重，加了 `s.alertedAt[簽名]`
  時間戳，同一個 (流程,節點,錯誤訊息) 簽名 24 小時內只報一次（用 workflow static data，
  跨執行持久化）
- 待聯絡提醒本來就一天只跑一次，不需要防吵

⚠️ 原本清單裡還有「KEIS 心跳檢查」，**08-25 當天兩度誤啟用引發假警報後已經整支刪除**，
不是這批的一員了，細節見 `incidents.md`。

**發文預告／發文成功（2026-08-25 晚間補漏）**：上面那批只補了「失敗類」，兩個**正常流程**
的推播被漏掉，結果廣告有正常發、使用者一則都收不到。補了三個節點
（腳本 `scripts/n8n_restore_publish_push.py`，可重跑）：

| 流程 | 新節點 | 接法 |
|---|---|---|
| 線A 掃描發文 | `Telegram 預告` | **串接** `組預告 → Telegram 預告 → 煞車 10 分鐘` |
| 線A 掃描發文 | `組發布通知 → Telegram 發布通知` | 並聯在成功終點 `Notion 記 KEIS 廣告ID` 之後 |
| 線C 重發輪替 | `組重發通知 → Telegram 重發通知` | 並聯在 `Notion 更新重發` 之後 |

⚠️ **預告為什麼是串接不是並聯**：並聯的話 Wait 節點一暫停，推播有可能排到 10 分鐘之後才送出，
預告就沒意義了。串接可行是因為下游 `重查狀態`／`判斷是否被停` 都用 `$('組預告')` 取值、
...
**判斷五：公買搶單存活檢查**（2026-09-14 加、09-18 確認 58 筆「正常」）：08:00~23:00 每小時看
`%LOCALAPPDATA%\keis-grab\inventory.csv` 有沒有在動，>25 分鐘沒動就直推 Telegram。
⚠️ 還沒被真實的死亡事件驗過。
⚠️ **改 `run.bat` 只能動 `python` 那行以下的內容**：cmd.exe 按位元組位置續讀 .bat，grab.py 還在
跑時動到前面幾行，它會從半行續讀、直接掉出迴圈——正好製造這支要防的災難。

**已決定不做｜「07:00 就開搶」**（2026-09-14 查證）：電腦 00:30 被 `MidnightAutoShutdown` 完全關機、
07:30 才開（BIOS 定時開機，Windows 喚醒排程叫不動全關機的機器），門市網路 07:22 才通 → 真缺口只有
07:22~07:31 約 9 分鐘且是最慢那層（60 秒）。要補得動 BIOS 或改休眠，代價大於效益。
**07:59~08:03 的 5 秒熱窗沒受影響**。
