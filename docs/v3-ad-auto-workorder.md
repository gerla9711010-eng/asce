...
2. **沒有 `KEIS廣告ID` 的**（v2 時期用 LINE 手動貼永慶網址建的舊資料，本來就不在 KEIS）→ 退回雙重確認：`property-management` 查無 **而且** 官網連結真的死了（`this.helpers.httpRequest` 打一次，HTTP ≥400 或頁面含 `已下架/物件不存在/已成交/查無此物件`）。
   ⚠️ 2026-08-05 起打的是 `永慶官網連結`。原本打 `來源連結`（KEIS `official_url`＝houseol），但 houseol 頁面會過期、實測全部 404，那道二次確認已經恆真＝保險絲失效；該欄位同日刪除。
...
## 20. Notion 在 v3 裡到底在幹嘛（2026-07-23 盤點，**不要手癢拿掉**）

線 D 做完後 Notion 跟 KEIS 廣告追蹤有大量欄位重疊，看起來像多餘的。**它不是資料庫，是決策紀錄本。**

自動化真正會**讀回來**的只有 6 欄：`案件編號`(線A判重)、`狀態`(線A煞車重查／線B撈清單)、`粉專貼文連結`(線B刪文)、`KEIS廣告ID`(線B關廣告)、`永慶官網連結`(線B舊資料二次確認、KEIS `adcase_url`)、`案名`(通知文字)。

> 2026-08-05：第 5 欄原本是 `來源連結`，該欄已刪除（存的是會過期的 houseol 網址），改成 `永慶官網連結`。
> 因此下面「純裝飾欄位」名單裡的 `永慶官網連結` 也不再是裝飾欄位。

**不可取代的兩個理由**：

1. **只有 Notion 記得「使用者決定不做的事」。** 被「停」攔下的案子狀態＝`取消`，KEIS 上完全沒有紀錄（根本沒發成功）。如果改用 KEIS adcases 判重，這些案子每 2 小時會被重新撈出來再發一次，使用者要一直喊停。
2. **10 分鐘煞車視窗需要一個可寫的暫存。** 那段時間 FB 還沒發文、KEIS 還沒建廣告，「停」指令只能寫進 Notion 狀態。

**純裝飾欄位（自動化從不讀，只給人手機上翻）**：總價、地址、格局、樓層、屋齡、社區名稱、主建物坪數、建物坪數、粉專文案、社團文案、永慶官網連結、KEIS同步、下架偵測時間。
